...
const CustomTooltip = ({
  ...
  if (active && payload && payload.length) {
    const data = (payload[0] as Record<string, unknown>)
      .payload as AnalysisItem & { fill: string };
    return (
        ...
        <div className="space-y-1 text-gray-500">
          ...
          <div className="flex gap-3 mt-2 pt-2 border-t border-gray-100">
            <span className="flex items-center gap-1">
              <HardDrive size={10} /> {data.metadata.file_size_kb} KB
            </span>
            <span className="uppercase bg-gray-100 px-1.5 py-0.5 rounded text-[9px] font-medium">
...
export default function ResultsView({
  ...
  const data = useMemo(() => {
    const rawData = analysis.map((item, index) => ({
      ...item,
      id: index,
  ...
  const { baseRangeX, baseRangeY, midX, midY } = useMemo(() => {
    if (data.length === 0)
      return { baseRangeX: 2, baseRangeY: 2, midX: 0, midY: 0 };
  ...
  const currentDomainX = [
    midX + center.x - currentRangeX / 2,
    midX + center.x + currentRangeX / 2,
  ];
  const currentDomainY = [
    midY + center.y - currentRangeY / 2,
    midY + center.y + currentRangeY / 2,
  ];
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      setZoom((prevZoom) => {
        const newZoom = Math.min(
          Math.max(prevZoom + delta * prevZoom, 0.5),
          20,
        );
        if (newZoom !== prevZoom) {
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (isDragging) {
      ...
      const domainPerPixelX =
        currentRangeX / chartContainerRef.current!.clientWidth;
      const domainPerPixelY =
        currentRangeY / chartContainerRef.current!.clientHeight;
      setCenter((prev) => ({
      ...
      <div className="flex justify-between items-end text-white">
        <div>
          ...
          <p className="text-white/70 text-sm">
            Organized {totalFiles} documents into {totalClusters} intelligent
            clusters.
          </p>
        </div>
        <button
            ...
            <div>
              ...
              <div className="flex items-baseline gap-1">
                <span className="text-xl font-bold">
                  {Math.round((summary.total_size_kb / 1024) * 10) / 10}
                </span>
        ...
        <div className="lg:col-span-2 bg-white/20 backdrop-blur-2xl rounded-3xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] border border-white/10 p-6 flex flex-col relative overflow-hidden">
          <div className="flex justify-between items-center mb-4">
            ...
            <button
              ...
              className="px-3 py-1 bg-white/10 rounded-lg hover:bg-white/20 text-white/80 transition-colors text-xs font-medium border border-white/10"
            >
              Reset View
            </button>
          </div>
        ...
        <div className="bg-white/20 backdrop-blur-2xl rounded-3xl shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] border border-white/10 p-6 flex flex-col overflow-hidden">
          ...
          <div className="overflow-y-auto pr-2 space-y-3 scrollbar-hide flex-1">
            {clusters.map((folder) => (
              <div
                key={folder}
                className="bg-white/5 hover:bg-white/10 rounded-xl p-3 border border-white/5 transition-colors group"
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2 overflow-hidden">
                    <span
                      className="w-2 h-2 rounded-full flex-shrink-0 shadow-[0_0_8px_rgba(255,255,255,0.4)]"
                      style={{ backgroundColor: clusterColorMap[folder] }}
                    />
                    <span
                      className="font-medium text-xs text-white/90 truncate"
                      title={folder}
                    >
                      {folder}
                    </span>
                ...
                <div className="space-y-1">
                  {analysis
                    ...
                    .map((file, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-2 text-[10px] text-white/50 truncate pl-4 group-hover:text-white/70 transition-colors"
                      >
